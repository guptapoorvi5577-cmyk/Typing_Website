...
export default function Practice() {
  ...
  function endTest(currentTyped = typed) {
    clearInterval(timerRef.current);
    setRunning(false);
    setFinished(true);

    const wordsTyped = currentTyped.trim().split(/\s+/).filter(Boolean);
    const referenceWords = passage.trim().split(/\s+/);
    ...
    const wpm = Math.round(wordsTyped.length / minutes);
    const accuracy =
      wordsTyped.length > 0
        ? Math.round((correct / wordsTyped.length) * 100 * 10) / 10
        : 0;
  ...
  const elapsed = TEST_DURATION - timeLeft;
  const liveWords = typed.trim().split(/\s+/).filter(Boolean);
  const liveWpm = elapsed > 0 ? Math.round(liveWords.length / (elapsed / 60)) : 0;
  const refWords = passage.split(/\s+/);
  const liveCorrect = liveWords.filter((w, i) => w === refWords[i]).length;
  const liveAccuracy =
    liveWords.length > 0
      ? Math.round((liveCorrect / liveWords.length) * 100 * 10) / 10
      : 0;
    ...
    >
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-8">
        ...
        <div className="rounded-2xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900 shadow-sm overflow-hidden">
          ...
          <div
            className="relative px-6 pt-6 pb-3 cursor-text select-none"
            onClick={focusInput}
          >
